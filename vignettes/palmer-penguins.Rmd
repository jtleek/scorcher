...
scorch_model <- initiate_scorch(dl) |>
  scorch_input("x") |>
  scorch_layer("fc1", "linear", in_features = 7, out_features = 16) |>
  scorch_layer("act1", "relu") |>
  scorch_layer("fc2", "linear", in_features = 16, out_features = 8) |>
  scorch_layer("act2", "relu") |>
  scorch_layer("fc3", "linear", in_features = 8, out_features = 3) |>
  scorch_output("fc3")
```
...
scorch_model <- scorch_model |>
  fit_scorch(num_epochs = 200, verbose = FALSE)
```
...
output <- scorch_model$nn_model(x_test)
